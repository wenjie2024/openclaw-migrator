    ...
    .action(async (options) => {
        try {
            ...
            if (options.source) {
                ...
                console.log(`📦 Archiving sources: (Auto-detecting...)`);
            }
...
program.command('import')
    ...
    .option('--minimal', 'Minimal restore (only Memory, Skills, Credentials)', false)
    .action(async (options) => {
        try {
            const password = getPassword(options);
            ...
            if (options.minimal) console.log("✨ Minimal restore mode enabled");
